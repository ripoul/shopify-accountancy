...
const WITH_PARAMS = '/shopify/install?shop=test.myshopify.com&hmac=abc&timestamp=123';
...
describe('ShopifyInstallPage', () => {
  ...
  it('shows missing params warning when required query params are absent', () => {
    ...
    expect(screen.getByText(/paramètres shopify manquants/i)).toBeInTheDocument();
  });

  it('shows loading state when all required params are present', () => {
    installStore.mockReturnValue(new Promise(() => {}));
    render(
  ...
  it('redirects to authorization_url on successful install', async () => {
    ...
    installStore.mockResolvedValue({ data: { authorization_url: 'https://shopify.com/oauth' } });
  ...
  it('shows API error messages on installStore failure with response data', async () => {
    installStore.mockRejectedValue({
  ...
  it('shows generic error message when no response data', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/impossible d'initier l'installation/i)).toBeInTheDocument();
    });
  });
