...
describe('LoginPage', () => {
  ...
  it('redirects to pending Shopify path after successful login', async () => {
    sessionStorage.setItem('shopify_pending_params', '/shopify/callback?shop=test');
    useAuth.mockReturnValue({ login: vi.fn().mockResolvedValue({}) });
    render(<MemoryRouter><LoginPage /></MemoryRouter>);
    ...
    await waitFor(() => {
      expect(mockNavigate).toHaveBeenCalledWith('/shopify/callback?shop=test', { replace: true });
  ...
  it('shows error from API detail on login failure', async () => {
    useAuth.mockReturnValue({
      login: vi.fn().mockRejectedValue({ response: { data: { detail: 'Identifiants invalides' } } }),
    });
    render(<MemoryRouter><LoginPage /></MemoryRouter>);
  ...
  it('shows generic error message when API returns no detail', async () => {
    useAuth.mockReturnValue({ login: vi.fn().mockRejectedValue({}) });
    render(<MemoryRouter><LoginPage /></MemoryRouter>);
  ...
  it('disables the submit button while loading', async () => {
    ...
    useAuth.mockReturnValue({ login: vi.fn().mockReturnValue(loginPromise) });
    render(<MemoryRouter><LoginPage /></MemoryRouter>);
