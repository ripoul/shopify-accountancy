...
const FULL_PARAMS = '/shopify/callback?shop=test.myshopify.com&code=mycode&hmac=myhmac&host=myhost&state=mystate&timestamp=123';
...
describe('ShopifyCallbackPage', () => {
  ...
  it('shows loading state when all required params are present', () => {
    connectStore.mockReturnValue(new Promise(() => {}));
    render(
  ...
  it('navigates to / with replace on successful connection', async () => {
    connectStore.mockResolvedValue({});
    render(
      <MemoryRouter initialEntries={[FULL_PARAMS]}>
        <ShopifyCallbackPage />
      </MemoryRouter>
  ...
  it('shows API error messages on connectStore failure with response data', async () => {
    connectStore.mockRejectedValue({
  ...
  it('shows generic error message when no response data', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/impossible de connecter la boutique/i)).toBeInTheDocument();
    });
  });
